...
# Uses 6WFF patch from /toppar_all36_carb_imlab.str and BGLC from
# top_all36_carb.rtf 
...
# Loukas's cellulose code and then patch with 6WFF. This way there is
...
while { ($rel_err > $targ_relerr) || ($trialnum > $max_att) } {
	...
	for {set i $deg_poly} {$i>1} {incr i -1} {
		...
		if {$i%2 == 1} { 
		    if {rand() < $acet_prob} {
			patch 6WFF $segment:$i 
			puts $fp "patch 6WFF $segment:$i"
