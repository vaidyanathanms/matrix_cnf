...
acet_perarr = [0.3,0.5] # fraction of acetylated cellulose
...
add_poly  = 'none' #'copoly' # # blend/homo/copoly/None
ex_ptype  = ['paa','pvp'] #['peo-ppo-peo'] # #p1,p2->blend;p1-p2-p3->copoly;p1->homo
ex_nch    = [15, 15] #[4] #[15, 15] # number of chains of each type
ex_nmon   = [10, 7] #[30] #[10, 7] # degree of polymerization of each type
ex_gaus   = [0.05, 0.2] #[0.2] #[0.05,0.2] # Gaussian tolerance for chains
...
scr_dir   = scr_dir + '/cnf'
#-------------------Loop acetylation--------------------------------------
for acet_val in acet_valarr:

    for acet_per in acet_perarr:
        ...
        if matrix.lower() != 'none':
            ...
            pack_sup  = scr_dir + '/bare_cnf' # final packed output super dir
            nmons = 0; nchains = 0 #safety net
        ...
        if acet_per != 0:
            ...
            print('Making unacetylated chains ..')
        # Use same function and tweak tolerance
        ...
        prm_file,itp_file,mol_infoarr = split_top_file_to_prmitp(acet_fyle,\
                                                                 pack_dir,main_dir)
