...
from omm_GPU import *
from omm_statedatareporter import modStateDataReporter
...
parser.add_argument('-zlayer', dest='zlayer', help='Vacuum layer along Z axis (optional)', default=None)
...
parser.add_argument('-ostr', metavar='STRFILE', dest='ostr', help='Output str file for CHARMM (optional)', default=None)
...
parser.add_argument('-wca', dest='wca', help='apply WCA potential (optional)', action='store_true', default=False)
parser.add_argument('-epscale', dest='epscale', help='Epsilon scaling in terms of tb and tc (optional)', action='store_true', default=None)
parser.add_argument('-skip', dest='skip', help='Skip all processing except writing pdb (optional)', action='store_true', default=False)
...
if inputs.vdw == 'Force-switch': system = vfswitch(system, psf, inputs, wca=args.wca, scale=args.epscale, params=params)
...
run_md = True
if args.wca:
    if hasattr(system, 'run_md'):
        run_md = system.run_md

if args.skip:
    print("skip this equilibration step...");
    run_md = False
...
avail_GPU = False
while not avail_GPU:
    print("checking GPU cards...")
    njob = get_njob()
    print('No.running Jobs: %d ...' % njob)
    if njob < 2:
        avail_GPU = True
    else:
        print("All GPU cards are occupied...")
...
if args.zlayer:
    ...
    C.z += float(args.zlayer)/10.0
    a = Quantity(np.zeros([3]), nanometers); a[0] = A.x * nanometers
    b = Quantity(np.zeros([3]), nanometers); b[1] = B.y * nanometers
    c = Quantity(np.zeros([3]), nanometers); c[2] = C.z * nanometers
...
# Energy minimization
if inputs.mini_nstep > 0 and not args.skip:
...
if inputs.nstep > 0 and run_md:
    ...
    simulation.reporters.append(
        modStateDataReporter(sys.stdout, inputs.nstout, step=True, time=True, potentialEnergy=True, temperature=True, progress=True,
                          remainingTime=True, speed=True, totalSteps=inputs.nstep, separator='\t', outFormat=['%-5s', '%13s', '%12d', '%9.3f', '%9.3f','%9s', '%9s'])
    )
    # Simulated Annealing?
...
if args.orst:
    ...
    if args.ostr:
        with open(args.ostr, 'w') as f:
            A,B,C = state.getPeriodicBoxVectors()
            lx = A.in_units_of(angstrom).x
            ly = B.in_units_of(angstrom).y
            lz = C.in_units_of(angstrom).z
            if lx == ly and ly == lz:
                boxtype = 'CUBIC'
            else:
                boxtype = 'TETRA'
            f.write('*Information about system\n*\ncalc A = {A}\ncalc B = {B}\ncalc C = {C}\nset bxtype = {boxtype}\n'.format(A=lx, B=ly, C=lz, boxtype=boxtype))
if args.ochk:
...
if args.opdb:
    ...
    topology = PDBFile('step3_charmm2omm.pdb').getTopology()
    PDBFile.writeFile(topology, crd, open(args.opdb, 'w'))
