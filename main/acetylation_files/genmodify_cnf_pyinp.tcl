...
# Set inputs
set deg_poly py_nmons
set acet_prob py_acprob
set carb_pos 6 ; #carbon position for substitution
set targ_relerr py_relerr ; # value between 0 and 1
set max_att py_maxatt ; # maximum attempt for acetylation
...
set fp [open "py_outdir/acetylation_py_acprob_mpy_acetval.dat" w]
puts $fp "Acetylation probability: $acet_prob"
puts $fp "Acetylation patch used: py_patch"
...
foreach n { 4 5 6} {
    ...
    coordpdb py_cnfmain/csff-elementary-fibril-$segment.pdb $segment
    guesscoord
...
while { ($rel_err > $targ_relerr) || ($trialnum > $max_att) } {
	...
	for {set i $deg_poly} {$i>1} {incr i -1} {
		...
		if {$i%2 == 1} { 
		    if {rand() < $acet_prob} {
			patch py_patch $segment:$i 
			puts $fp "patch py_patch $segment:$i"
