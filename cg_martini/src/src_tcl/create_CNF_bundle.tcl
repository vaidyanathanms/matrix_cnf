...
foreach file $files {
  ...
  if {$file_counter eq 1} {
    ...
    set sel [atomselect top all]
    $sel moveby [list [expr {$d * -1}] 0 0]
  } elseif {$file_counter eq 4} {
    set sel [atomselect top all]
    $sel moveby [list [expr {$d * 0.5 * -1}] [expr {$d * 0.5}] 0]
  } elseif {$file_counter eq 5} {
    set sel [atomselect top all]
    $sel moveby [list [expr {$d * 0.5}] [expr {$d * 0.5}] 0]
  } elseif {$file_counter eq 6} {
    set sel [atomselect top all]
    $sel moveby [list [expr {$d * 0.5 * -1}] [expr {$d * 0.5 * -1}] 0]
  } elseif {$file_counter eq 7} {
    set sel [atomselect top all]
    $sel moveby [list [expr {$d * 0.5}] [expr {$d * 0.5 * -1}] 0]
  }
  $sel writepdb $new_file
}
...
set final_sel [::TopoTools::selections2mol "$sel_0 $sel_1 $sel_2 $sel_3 $sel_4 $sel_5 $sel_6"]
